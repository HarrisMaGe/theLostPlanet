...
bool GameLayer::init() {
	...
	this->addChild(myBackGround->createBackGround_1(),1);
	SimpleAudioEngine::getInstance()->playBackgroundMusic("music/bgmusic_level1.mp3", -1);
	...
	//Boss激光创建
	this->schedule(schedule_selector(GameLayer::scheduleBossLaser1));
	//Boss激光碰撞检测
	this->schedule(schedule_selector(GameLayer::scheduleBossLaserShoot1));
	...
	//敌机
	myEnemyLayer = EnemyLayer::create(myPlayerLayer->player);
	this->addChild(myEnemyLayer, 1);
	//this->schedule(schedule_selector(GameLayer::enemySch),1);
	

	//敌机子弹
	myEnemyBulletLayer = EnemyBulletLayer::create(myPlayerLayer->player);
	this->addChild(myEnemyBulletLayer,4);
	 this->schedule(schedule_selector(GameLayer::enemyBulletManagerA), 1);
	this->schedule(schedule_selector(GameLayer::enemyBulletManagerB), 1);
	this->schedule(schedule_selector(GameLayer::enemyBulletManagerC),2);
	this->schedule(schedule_selector(GameLayer::enemyBulletManagerD), 1.5);


	//敌机
	//this->schedule(schedule_selector(GameLayer::newEnemy), 0.5);
	//this->schedule(schedule_selector(GameLayer::moveEnemy), 0.01);
...
void GameLayer::menuBack(Ref* pSender)
{
...
void  GameLayer::moveEnemy(float t) {
...
//player和敌机
		...
		{//两百点血量
			if (nowenemy->getTag() <= 2800)
			{
			 ...
			 if (rand() % 1000 > 999)//速度
				...
				//通关检测
				this->scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer::scheduleOnceWin), 3);
	...
	{ 
		...
		//死亡界面
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer::scheduleOnceDeath), 3);
	}
	...
	if (this->getChildByTag(221) != NULL)
	{
		...
		for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
		{
			...
			{
				laser->setPosition(nowenemy->getPosition().x, nowenemy->getPosition().y -390);
...
//删除坐标精灵，释放内存
void GameLayer::del(Node * pSender)
{
...
//爆炸效果
void GameLayer::newBomb(int x, int  y)
{
...
//	SimpleAudioEngine::getInstance()->playEffect("music/boom.wav");
	...
	Action * act = Sequence::create(
		...
		CCCallFuncN::create(sprite, callfuncN_selector(GameLayer::del)), //删除精灵
		NULL);
...
//Boss爆炸效果
void GameLayer::newBossBomb(int x, int  y)
{
...
//闪电效果
void GameLayer::Light(int x, int  y)
{
	Vector<SpriteFrame*> allframe;
...
//Boos激光
void GameLayer::Laser(Node *player)
{
	Vector<SpriteFrame*> allframe;
...
//护盾
void GameLayer::shield_F(Node *player)
{
	Vector<SpriteFrame*> allframe;
...
//机尾火焰
void GameLayer::fire_F(Node *player)
{
	Vector<SpriteFrame*> allframe;
...
void GameLayer::nuclear(int x, int y) {
...
//奖励消除调度器
void GameLayer::scheduleOnceFOODManager_1() {
	//子弹变化
	if (Food1==1)
	{
		//改变子弹作为奖励
		...
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer::scheduleOnceFOOD1), 5);
	...
	{
		unschedule(schedule_selector(GameLayer::scheduleOnceFOOD1));
		myPlayerBulletLayer->stopBulletForPlayer();
...
void GameLayer::scheduleOnceFOODManager_2() {
	...
	if (Food2 == 1)
	{
		...
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer::scheduleOnceFOOD2), 5);
	...
	{
		unschedule(schedule_selector(GameLayer::scheduleOnceFOOD2));
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer::scheduleOnceFOOD2), 5);
	}
}
void GameLayer::scheduleOnceFOODManager_3() {

	//无敌
	if (Food3 == 1)
	{
		shield_F(myPlayerLayer->player);
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer::scheduleOnceFOOD3), 3);
		Food3++;
	}
	else if (Food3 == 0)
	{
	...
	{
		unschedule(schedule_selector(GameLayer::scheduleOnceFOOD3));
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer::scheduleOnceFOOD3), 3);
	}
...
void GameLayer::scheduleOnceFOOD1(float t){
	myPlayerBulletLayer->stopBulletForPlayer();
	myPlayerBulletLayer->createBulletForPlayer( 1, 0.2, 500);
	Food1 = 0;

}
void GameLayer::scheduleOnceFOOD2(float t) {
...
void GameLayer::scheduleOnceFOOD3(float t) {
...
//敌机子弹调度器
//直线发射
void GameLayer::enemyBulletManagerA(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		if (nowenemy->getTag() >1500)
		{
			myEnemyBulletLayer->newEnemyBullet(nowenemy, 1, 800);
		}

		if (nowenemy->getTag() <= 1000 && nowenemy->getTag()>500)
		{
			myEnemyBulletLayer->newEnemyBullet(nowenemy, 2, 400);
		}
...
//三发子弹
void GameLayer::enemyBulletManagerB(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		if (nowenemy->getTag() >1500)
		{
			myEnemyBulletLayer->newEnemyBullet(nowenemy, 2, 600);
		}
...
//5发散  Boos和Enemy3
void GameLayer::enemyBulletManagerC(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		if ( nowenemy->getTag()>1000)
		{
			myEnemyBulletLayer->newEnemyBullet(nowenemy, 3, 400);
		}
...
void GameLayer::enemyBulletManagerD(float t) {
...
void GameLayer::scheduleOnceDeath(float t) {
...
void GameLayer::scheduleBossLaser1(float t) {
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		auto nowenemy = myEnemyLayer->myEnemyStlyle->allEnemy.at(i);
		if (nowenemy->getTag() > 1500)
		{  //关闭Boss出现监听调度器
			this->unschedule(schedule_selector(GameLayer::scheduleBossLaser1));
			//5秒后启动激光
			this->scheduleOnce(schedule_selector(GameLayer::scheduleBossLaser2),5);
		}
	}
...
//激光启动调度器
void GameLayer::scheduleBossLaser2(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		{ //启动激光
			Laser(nowenemy);
			//5秒后关闭激光
			this->scheduleOnce(schedule_selector(GameLayer::scheduleBossLaser3), 5);
		}
	}

}

void GameLayer::scheduleBossLaser3(float t) {
	//关闭激光
	if (this->getChildByTag(221) != NULL)
	{  
		auto *laser = this->getChildByTag(221);
...
//激光碰撞检测调度器
void GameLayer::scheduleBossLaserShoot1(float t) {
	...
	if (this->getChildByTag(221) != NULL)
	{
		...
		for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
		{
			...
			{
				laser->setPosition(nowenemy->getPosition().x, nowenemy->getPosition().y - 390);
				Rect laserRect(nowenemy->getPosition().x - 17, nowenemy->getPosition().y - 300 - 390, 34, 600);
				if (laserRect.intersectsRect(rectPlayer))
				{
					this->unschedule(schedule_selector(GameLayer::scheduleBossLaserShoot1));
					...
					//0.5秒重新调度
					this->scheduleOnce(schedule_selector(GameLayer::scheduleBossShoot2),0.5);
				}
...
//激光碰撞扣血调度器
void GameLayer::scheduleBossShoot2(float t) {
...
void  GameLayer::scheduleOnceWin(float t) {
	Size winSize = Director::getInstance()->getWinSize();
	...
	this->scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer::scheduleOnceWin2), 5);
}

//跳转界面
void  GameLayer::scheduleOnceWin2(float t) {
