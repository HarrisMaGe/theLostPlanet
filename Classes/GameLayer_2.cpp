...
Scene* GameLayer_2::createScene()
{

	auto scene = Scene::create();


	auto layer = GameLayer_2::create();
...
bool GameLayer_2::init() {
	...
	this->addChild(myBackGround->createBackGround_2(), 1);
	SimpleAudioEngine::getInstance()->playBackgroundMusic("music/bgmusic_stage2.mp3", -1);
	...
	auto reStartItem = MenuItemImage::create(
		...
		CC_CALLBACK_1(GameLayer_2::menuCallback, this));
	...
	//Boss激光创建
	this->schedule(schedule_selector(GameLayer_2::scheduleBossLaser1));
	//Boss激光碰撞检测
	this->schedule(schedule_selector(GameLayer_2::scheduleBossLaserShoot1));
	...
	//敌机
	myEnemyLayer = EnemyLayer_2::create(myPlayerLayer->player);
	this->addChild(myEnemyLayer, 1);
	//this->schedule(schedule_selector(GameLayer_2::enemySch),1);


	//敌机子弹
	myEnemyBulletLayer = EnemyBulletLayer::create(myPlayerLayer->player);
	this->addChild(myEnemyBulletLayer, 4);
	this->schedule(schedule_selector(GameLayer_2::enemyBulletManagerA), 1);
	this->schedule(schedule_selector(GameLayer_2::enemyBulletManagerB), 1);
	this->schedule(schedule_selector(GameLayer_2::enemyBulletManagerC), 2);
	this->schedule(schedule_selector(GameLayer_2::enemyBulletManagerD), 1.5);


	//敌机
	//this->schedule(schedule_selector(GameLayer_2::newEnemy), 0.5);
	//this->schedule(schedule_selector(GameLayer_2::moveEnemy), 0.01);
...
//回调函数
void GameLayer_2::menuCallback(Ref* pSender)
{
...
void GameLayer_2::menureStartCallBack(Ref* pSender)
{
	Director::getInstance()->replaceScene(GameLayer_2::createScene());

}
void GameLayer_2::menuBack(Ref* pSender)
{
...
/*
void  GameLayer_2::newEnemy(float t) {
...
void  GameLayer_2::moveEnemy(float t) {
...
void GameLayer_2::update(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		{//两百八十点血量
			if (nowenemy->getTag() <= 2720)
			{
				...
				//通关检测
				this->scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer_2::scheduleOnceWin), 3);
	...
	{  
		...
		//调度器关闭
		unschedule(schedule_selector(GameLayer_2::update));
		...
		//死亡界面
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer_2::scheduleOnceDeath), 3);
	}
	...
	if (this->getChildByTag(221) != NULL)
	{
		...
		for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
		{
			...
			{
				laser->setPosition(nowenemy->getPosition().x, nowenemy->getPosition().y - 470);
...
//删除坐标精灵，释放内存
void GameLayer_2::del(Node * pSender)
{
...
//爆炸效果
void GameLayer_2::newBomb(int x, int  y)
{
	//爆炸声效
	...
	Action * act = Sequence::create(
		...
		CCCallFuncN::create(sprite, callfuncN_selector(GameLayer_2::del)), //删除精灵
		NULL);
...
//Boss爆炸效果
void GameLayer_2::newBossBomb(int x, int  y)
{
...
//闪电效果
void GameLayer_2::Light(int x, int  y)
{
	Vector<SpriteFrame*> allframe;
...
//Boos激光
void GameLayer_2::Laser(Node *player)
{
	Vector<SpriteFrame*> allframe;
...
//	SimpleAudioEngine::getInstance()->playEffect("music/click.wav", false, 1, 0, 0.3);
	...
	Laser->setPosition(player->getPosition().x, player->getPosition().y - 510);
...
//护盾
void GameLayer_2::shield_F(Node *player)
{
	Vector<SpriteFrame*> allframe;
...
//机尾火焰
void GameLayer_2::fire_F(Node *player)
{
	Vector<SpriteFrame*> allframe;
...
void GameLayer_2::nuclear(int x, int y) {
...
//奖励消除调度器
void GameLayer_2::scheduleOnceFOODManager_1() {
	//子弹变化
	if (Food1 == 1)
	{
		//改变子弹作为奖励
		...
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer_2::scheduleOnceFOOD1), 5);
	...
	{
		unschedule(schedule_selector(GameLayer_2::scheduleOnceFOOD1));
		myPlayerBulletLayer->stopBulletForPlayer();
...
void GameLayer_2::scheduleOnceFOODManager_2() {
	...
	if (Food2 == 1)
	{
		...
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer_2::scheduleOnceFOOD2), 5);
	...
	{
		unschedule(schedule_selector(GameLayer_2::scheduleOnceFOOD2));
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer_2::scheduleOnceFOOD2), 5);
	}
}
void GameLayer_2::scheduleOnceFOODManager_3() {

	//无敌
	if (Food3 == 1)
	{
		shield_F(myPlayerLayer->player);
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer_2::scheduleOnceFOOD3), 3);
		Food3++;
	}
	else if (Food3 == 0)
	{
	...
	{
		unschedule(schedule_selector(GameLayer_2::scheduleOnceFOOD3));
		scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer_2::scheduleOnceFOOD3), 3);
	}
...
void GameLayer_2::scheduleOnceFOOD1(float t) {
	myPlayerBulletLayer->stopBulletForPlayer();
	myPlayerBulletLayer->createBulletForPlayer(1, 0.2, 500);
	Food1 = 0;

}
void GameLayer_2::scheduleOnceFOOD2(float t) {
...
void GameLayer_2::scheduleOnceFOOD3(float t) {
...
//敌机子弹调度器
//直线发射
void GameLayer_2::enemyBulletManagerA(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		{
			myEnemyBulletLayer->newEnemyBullet(nowenemy, 4, 400);
		}
		...
		{
			myEnemyBulletLayer->newEnemyBullet(nowenemy, 1, 400);
		}
...
//三发子弹
void GameLayer_2::enemyBulletManagerB(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		if (nowenemy->getTag() >1500)
		{
			myEnemyBulletLayer->newEnemyBullet(nowenemy, 7, 600);
		}
...
//5发散  Boos和Enemy3
void GameLayer_2::enemyBulletManagerC(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		if (nowenemy->getTag()>1000)
		{
			myEnemyBulletLayer->newEnemyBullet(nowenemy, 6, 400);
		}
...
void GameLayer_2::enemyBulletManagerD(float t) {
...
void GameLayer_2::scheduleOnceDeath(float t) {
...
void GameLayer_2::scheduleBossLaser1(float t) {
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		auto nowenemy = myEnemyLayer->myEnemyStlyle->allEnemy.at(i);
		if (nowenemy->getTag() > 1500)
		{  //关闭Boss出现监听调度器
			this->unschedule(schedule_selector(GameLayer_2::scheduleBossLaser1));
			//5秒后启动激光
			this->scheduleOnce(schedule_selector(GameLayer_2::scheduleBossLaser2), 5);
		}
	}
...
//激光启动调度器
void GameLayer_2::scheduleBossLaser2(float t) {
	...
	for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
	{
		...
		{ //启动激光
			Laser(nowenemy);
			//5秒后关闭激光
			this->scheduleOnce(schedule_selector(GameLayer_2::scheduleBossLaser3), 5);
		}
	}

}

void GameLayer_2::scheduleBossLaser3(float t) {
	//关闭激光
	if (this->getChildByTag(221) != NULL)
	{
		auto *laser = this->getChildByTag(221);
...
//激光碰撞检测调度器
void GameLayer_2::scheduleBossLaserShoot1(float t) {
	...
	if (this->getChildByTag(221) != NULL)
	{
		...
		for (int i = 0;i < myEnemyLayer->myEnemyStlyle->allEnemy.size();i++)
		{
			...
			{
				laser->setPosition(nowenemy->getPosition().x, nowenemy->getPosition().y - 470);
				Rect laserRect(nowenemy->getPosition().x - 17, nowenemy->getPosition().y - 300 - 470, 34, 600);
				if (laserRect.intersectsRect(rectPlayer))
				{
					this->unschedule(schedule_selector(GameLayer_2::scheduleBossLaserShoot1));
					...
					//0.5秒重新调度
					this->scheduleOnce(schedule_selector(GameLayer_2::scheduleBossShoot2), 0.5);
				}
...
//激光碰撞扣血调度器
void GameLayer_2::scheduleBossShoot2(float t) {
...
void  GameLayer_2::scheduleOnceWin(float t) {
	Size winSize = Director::getInstance()->getWinSize();
	...
	this->scheduleOnce(CC_SCHEDULE_SELECTOR(GameLayer_2::scheduleOnceWin2), 3);
}

//跳转界面
void  GameLayer_2::scheduleOnceWin2(float t) {
